#!/usr/bin/env python3
"""
MME Hallucination Evaluation with UVC Intervention for MMaDA.
...
def main():
    ...
    ap.add_argument("--num_heads", type=int, default=16, help="K")
    ap.add_argument("--alpha", type=float, default=44, help="Global-scale intervention strength")
    ...
    for task_name in tasks:
        ...
        for idx, (img_name, question, gt_answer) in enumerate(tqdm(items, desc=task_name)):
            ...
            try:
                ...
                qseed = base_seed + idx
                random.seed(qseed)
                np.random.seed(qseed % (2**32 - 1))
                torch.manual_seed(qseed)
                if device.type == "cuda":
                    torch.cuda.manual_seed_all(qseed)
