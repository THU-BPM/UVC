#!/usr/bin/env python3
"""
Extract global-scale degraded activation vectors for Lumina-DiMOO.

  - Only YES samples
  - Question: "What is [object] in the image?" (object name IN prompt)
  - Globally degraded image (GaussianBlur by default)
...
def eval_model(args):
    ...
    for line in tqdm(yes_samples[:args.length]):
        ...
        # global-scale degraded image
        raw_image = Image.open(os.path.join(args.image_folder, image_file)).convert("RGB")
        image = raw_image.filter(ImageFilter.GaussianBlur(radius=args.blur_radius))
    ...
    print(f"Saved {len(all_head_wise_activations)} global-scale degraded vectors -> {args.output}")
