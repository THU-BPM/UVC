#!/usr/bin/env python3
"""
Extract clean-reference activation vectors for Lumina-DiMOO.
...
def eval_model(args):
    ...
    print(f"Processing clean-reference vectors  |  YES samples: {len(yes_questions)} / {len(questions)}")
    print(f"  Question = 'What is the object in the image?'")
    print(f"  Answer = regex-extracted object name (GT) appended to prompt")
    print(f"  Image = clean,  gen region = ALL MASK")
    print(f"  gen_length = {gen_length},  pre-hook on attn_out INPUT")
    ...
    for line in tqdm(yes_questions[:args.length]):
        ...
        # extract object name from original POPE question
        match = object_pattern.search(orig_qs)
        object_name = match.group(1).strip() if match else "object"
        ...
        # hook-based activation extraction (PRE-HOOK: attn_out input)
        outputs_dict = {}
